...
export default function Home() {
  useEffect(()=>{
    ...
    function loose() {
      ...
      // var path = MorphSVGPlugin.convertToPath("#arc");
      let myPath = document.querySelector("#arc");


      gsap.to([newArrow], 0.5, {
        force3D: true,
        bezier: {
          type: "cubic",
          morphSvg:"#arc",
    ...
    function hitTest() {
      ...
      let tween = gsap.to(".bullseye", {rotation: 360, duration: 5, ease: "elastic"});
      var arrow = tween.targets()[0];
      console.log(arrow._gsTransform,'arrw')
      var transform = gsap.getProperty('#arrow');
      console.log(transform('y'))
      var radians = transform * Math.PI / 180;
      var arrowSegment = {
        x1: transform('x'),
        y1: transform('y'),
    ...
    function showMessage(selector) {
      ...
      // gsap.staggerFromTo(selector + " path", .5, {
      //   rotation: -5,
      //   scale: 0,
      ...
      gsap.to(selector+" path", {
        stagger: { // wrap advanced options in an object
          ...
          grid: "auto",
          ease: "power2.inOut",
          rotation:-5// Repeats immediately, not waiting for the other staggered animations to finish
        }
      });
      // gsap.staggerTo(selector + " path", .3, {
      //   delay: 2,
      //   rotation: 20,
      //   scale: 0,
      //   ease: Back.easeIn
...
<circle className="point" r="7" fill="purple" opacity=".4"/> --> */}
</svg>
   </div>
